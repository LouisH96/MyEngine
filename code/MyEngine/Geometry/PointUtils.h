...
namespace MyEngine
{
	namespace Geometry
	{
		...
		public:
			template<typename T>
			static void GetBounds2DPoints(const Array<Vector2<T>>& points, Vector2<T>& min, Vector2<T>& max);
			...
			template<typename T> static void Translate(Array<Vector3<T>>& points, const Vector3<T>& translation);
		};

		//---| Definitions |---

		template <typename T>
		void PointUtils::GetBounds2DPoints(const Array<Vector2<T>>& points, Vector2<T>& min, Vector2<T>& max)
		{
		...
		template <typename T>
		void PointUtils::Center2DPoints(Array<Vector2<T>>& points)
		{
			Vector2<T> min, max;
		...
		template<typename T>
		void PointUtils::Scale(Array<Vector2<T>>& points, T scale)
		{
			for (int i = 0; i < points.GetSize(); i++)
				points[i] *= scale;
		}
		...
		template <typename T>
		void PointUtils::Scale(Array<Vector3<T>>& points, T scale)
		{
			for (int i = 0; i < points.GetSize(); i++)
				points[i] *= scale;
		}
