...
void Io::Ttf::TableDirectory::Read(const Bin::BigBinReader& reader)
{
...
void Io::Ttf::TableDirectory::Print() const
{
	for (int i = 0; i < TAG_LENGTH; i++)
		std::cout << m_Tag[i];
	std::cout << "\t|" << m_CheckSum << "\t|";
...
void Io::Ttf::TableDirectory::PrintHeader()
{
	std::cout << "___| TableDirectory |___\n";
	std::cout << "Tag\t|CheckSum\t|Offset\t|Length\n";
}

std::string Io::Ttf::TableDirectory::GetTag() const
{
