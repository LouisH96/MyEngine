...
public:
	...
	Data& Get(unsigned idx) { return m_CpuBuffer.Get(idx); }
...
void InvalidateBuffer<Data>::ActivateVertexBuffer(unsigned slot)
{
	if (m_CpuBuffer.HasChanged())
	{
		...
		m_GpuBuffer.EnsureCapacity(m_CpuBuffer.GetSize());
...
template<typename Data>
inline void InvalidateBuffer<Data>::ActivateFromStart(unsigned slot)
{
	if (m_CpuBuffer.HasChanged())
	{
		m_CpuBuffer.ClearChangedFlag();
		m_GpuBuffer.EnsureCapacity(m_CpuBuffer.GetEndIdx());
		m_GpuBuffer.CopyData(m_CpuBuffer.SeeData(), m_CpuBuffer.GetEndIdx());
	}
	m_GpuBuffer.Activate(slot);
}
