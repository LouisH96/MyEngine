...
void UiImageRenderer::OnCanvasResized(const App::ResizedEvent&)
{
	m_2DivScreenSize = Calc2InvScreenSize();
}

void UiImageRenderer::Render()
{
...
void UiImageRenderer::Add(const RectFloat& screenSpace, const Float2& uvLeftTop, const Float2& uvRightBot, unsigned textureId)
{
...
Float2 UiImageRenderer::Calc2InvScreenSize()
{
	return Float2{ 2 } / Globals::pCanvas->GetSize();
}

unsigned UiImageRenderer::AddTexture(Rendering::Texture&& texture)
{
	m_Groups.Add(TextureGroup{ std::move(texture), 0 });
	return m_Groups.GetSize() - 1;
}

void UiImageRenderer::ReplaceTexture(unsigned textureId, Rendering::Texture&& texture)
{
